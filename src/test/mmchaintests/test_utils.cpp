...
void initState(){
    ...
    const std::string dirMMChain = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<MMChainState>(new MMChainState(dev::u256(0), MMChainState::openDB(dirMMChain, hashDB, dev::WithExisting::Trust), dirMMChain + "/mmchainDB", dev::eth::BaseState::Empty));
...
MMChainTransaction createMMChainTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout){
    MMChainTransaction txEth;
    if(recipient == dev::Address()){
        txEth = MMChainTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = MMChainTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
...
std::pair<std::vector<ResultExecute>, ByteCodeExecResult> executeBC(std::vector<MMChainTransaction> txs){
