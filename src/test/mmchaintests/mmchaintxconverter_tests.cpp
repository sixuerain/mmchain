...
void checkResult(bool isCreation, std::vector<MMChainTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    MMChainTxConverter converter(transaction, NULL);
    ExtractMMChainTX mmchainTx;
    BOOST_CHECK(converter.extractionMMChainTransactions(mmchainTx));
    std::vector<MMChainTransaction> result = mmchainTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(mmchaintxconverter_tests, TestingSetup)
