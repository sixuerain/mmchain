...
void Shutdown()
{
    ...
    if (pwalletMain)
    {
        StakeMMChains(false, pwalletMain);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/mmchain/mmchain>";
    const std::string URL_WEBSITE = "<https://mmchain.io>";
...
//////////////////////////////////////////////////////////////////////
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                dev::eth::Ethash::init();
                boost::filesystem::path mmchainStateDir = GetDataDir() / "stateMMChain";
                bool fStatus = boost::filesystem::exists(mmchainStateDir);
                const std::string dirMMChain(mmchainStateDir.string());
                const dev::h256 hashDB(dev::sha3(dev::rlp("")));
                dev::eth::BaseState existsMMChainstate = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
                globalState = std::unique_ptr<MMChainState>(new MMChainState(dev::u256(0), MMChainState::openDB(dirMMChain, hashDB, dev::WithExisting::Trust), dirMMChain, existsMMChainstate));
    ...
    if (!GetBoolArg("-staking", DEFAULT_STAKE))
        ...
        StakeMMChains(true, pwalletMain);
#endif
    // ********************************************************* Step 12: finished
