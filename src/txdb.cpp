    ...
    while (pcursor->Valid()) {
        ...
        if (pcursor->GetKey(key) && key.first == DB_BLOCK_INDEX) {
            ...
            if (pcursor->GetValue(diskindex)) {
                ...
                pindexNew->hashStateRoot  = diskindex.hashStateRoot; // mmchain
                pindexNew->hashUTXORoot   = diskindex.hashUTXORoot; // mmchain
                pindexNew->nStakeModifier = diskindex.nStakeModifier;
                pindexNew->prevoutStake   = diskindex.prevoutStake;
                pindexNew->vchBlockSig    = diskindex.vchBlockSig; // mmchain
