...
#define paternVersion "mmchain-([0-9]+\\.)?([0-9]+\\.)?([0-9]+)-"

MMChainVersionChecker::MMChainVersionChecker(QObject *parent) : QObject(parent)
{
    currentVersion = Version(CLIENT_VERSION_MAJOR, CLIENT_VERSION_MINOR, CLIENT_VERSION_REVISION);
}

MMChainVersionChecker::~MMChainVersionChecker()
{

}

bool MMChainVersionChecker::newVersionAvailable()
{
...
QList<Version> MMChainVersionChecker::getVersions()
{
    QNetworkAccessManager manager;
    QNetworkReply *response = manager.get(QNetworkRequest(QUrl(MMC_RELEASES)));
    QEventLoop event;
...
Version MMChainVersionChecker::getMaxReleaseVersion()
{
