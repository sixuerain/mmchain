...
#include "titlebar.h"
#include "mmchainversionchecker.h"
    ...
    platformStyle(_platformStyle)
{
    ...
    mmchainVersionChecker = new MMChainVersionChecker(this);

    if(mmchainVersionChecker->newVersionAvailable())
    {
        QString link = QString("<a href=%1>%2</a>").arg(MMC_RELEASES, MMC_RELEASES);
        QString message(tr("New version of MMChain wallet is available on the MMChain source code repository: <br /> %1. <br />It is recommended to download it and update this application").arg(link));
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a MMChain address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and mmchain: URIs)"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your MMChain addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified MMChain addresses"));
    ...
    openAction->setStatusTip(tr("Open a mmchain: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible MMChain command-line options").arg(tr(PACKAGE_NAME)));
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to MMChain network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
{
    QString strTitle = tr("MMChain"); // default title
