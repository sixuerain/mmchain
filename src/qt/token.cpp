...
bool ToHash160(const std::string& strMMChainAddress, std::string& strHash160)
{
    CBitcoinAddress mmchainAddress(strMMChainAddress);
    if(mmchainAddress.IsValid()){
        CKeyID keyid;
        mmchainAddress.GetKeyID(keyid);
...
bool ToMMChainAddress(const std::string& strHash160, std::string& strMMChainAddress)
{
    uint160 key(ParseHex(strHash160.c_str()));
    CKeyID keyid(key);
    CBitcoinAddress mmchainAddress;
    mmchainAddress.Set(keyid);
    if(mmchainAddress.IsValid()){
        strMMChainAddress = mmchainAddress.ToString();
        return true;
    }
    return false;
}
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToMMChainAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToMMChainAddress(tokenEvent.receiver, tokenEvent.receiver);
