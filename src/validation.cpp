...
#if defined(NDEBUG)
# error "MMChain cannot be compiled without assertions."
#endif
...
std::unique_ptr<MMChainState> globalState;
...
const std::string strMessageMagic = "MMChain Signed Message:\n";
...
bool CheckTransaction(const CTransaction& tx, CValidationState &state, bool fCheckDuplicateInputs)
{
    ...
    if (::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > MAX_TRANSACTION_BASE_SIZE ||
        ::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // mmchain
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        if(tx.HasCreateOrCall()){
            ...
            MMChainDGP mmchainDGP(globalState.get(), fGettingValuesDGP);
            uint64_t minGasPrice = mmchainDGP.getMinGasPrice(chainActive.Tip()->nHeight + 1);
            uint64_t blockGasLimit = mmchainDGP.getBlockGasLimit(chainActive.Tip()->nHeight + 1);
            size_t count = 0;
            for(const CTxOut& o : tx.vout)
                count += o.scriptPubKey.HasOpCreate() || o.scriptPubKey.HasOpCall() ? 1 : 0;
            MMChainTxConverter converter(tx, NULL);
            ExtractMMChainTX resultConverter;
            if(!converter.extractionMMChainTransactions(resultConverter)){
            ...
            std::vector<MMChainTransaction> mmchainTransactions = resultConverter.first;
            std::vector<EthTransactionParams> mmchainETP = resultConverter.second;

            dev::u256 sumGas = dev::u256(0);
            dev::u256 gasAllTxs = dev::u256(0);
            for(MMChainTransaction mmchainTransaction : mmchainTransactions){
                sumGas += mmchainTransaction.gas() * mmchainTransaction.gasPrice();
                ...
                if(txMinGasPrice != 0) {
                    txMinGasPrice = std::min(txMinGasPrice, mmchainTransaction.gasPrice());
                } else {
                    txMinGasPrice = mmchainTransaction.gasPrice();
                }
                VersionVM v = mmchainTransaction.getVersion();
                if(v.format!=0)
                ...
                if(mmchainTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
                ...
                if(mmchainTransaction.gas() > UINT32_MAX)
                ...
                gasAllTxs += mmchainTransaction.gas();
                ...
                if(v.rootVM!=0 && (uint64_t)mmchainTransaction.gasPrice() < minGasPrice)
            ...
            if(!CheckMinGasPrice(mmchainETP, minGasPrice))
                return state.DoS(100, false, REJECT_INVALID, "bad-txns-small-gasprice");

            if(count > mmchainTransactions.size())
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s MMC additional fees, %d delta bytes\n",
...
static DisconnectResult DisconnectBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // mmchain
    globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // mmchain

    if(pfClean == NULL && fLogEvents){
        boost::filesystem::path stateDir = GetDataDir() / "stateMMChain";
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit){
    ...
    MMChainTransaction callTransaction(0, 1, dev::u256(gasLimit), addrContract, opcode, dev::u256(0));
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());

    
    ByteCodeExec exec(block, std::vector<MMChainTransaction>(1, callTransaction), blockGasLimit);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path mmchainDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(mmchainDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(mmchainDir.string(), std::ios::in | std::ios::out);
...
bool MMChainTxConverter::extractionMMChainTransactions(ExtractMMChainTX& mmchaintx){
    std::vector<MMChainTransaction> resultTX;
    ...
    mmchaintx = std::make_pair(resultTX, resultETP);
    return true;
}

bool MMChainTxConverter::receiveStack(const CScript& scriptPubKey){
...
bool MMChainTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
MMChainTransaction MMChainTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    MMChainTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = MMChainTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = MMChainTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // mmchain
    MMChainDGP mmchainDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setMMChainSchedule(mmchainDGP.getGasSchedule(pindex->nHeight + 1));
    uint32_t sizeBlockDGP = mmchainDGP.getBlockSize(pindex->nHeight + 1);
    uint64_t minGasPrice = mmchainDGP.getMinGasPrice(pindex->nHeight + 1);
    uint64_t blockGasLimit = mmchainDGP.getBlockGasLimit(pindex->nHeight + 1);
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // mmchain
...
///////////////////////////////////////////////////////////////////////////////////////// mmchain
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            MMChainTxConverter convert(tx, &view, &block.vtx);

            ExtractMMChainTX resultConvertMMChainTX;
            if(!convert.extractionMMChainTransactions(resultConvertMMChainTX)){
            ...
            if(!CheckMinGasPrice(resultConvertMMChainTX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertMMChainTX.first, blockGasLimit);
            ...
            for(MMChainTransaction& qtx : resultConvertMMChainTX.first){
            ...
            if (fLogEvents)
            {
                for(size_t k = 0; k < resultConvertMMChainTX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{block.GetHash(), uint32_t(pindex->nHeight), tx.GetHash(), uint32_t(i), resultConvertMMChainTX.first[k].from(), resultConvertMMChainTX.first[k].to(),
...
//////////////////////////////////////////////////////////////////
    ...
    if(pindex->nHeight > 5000 && !Params().GetConsensus().fPoSNoRetargeting) {
        ...
        if(pindex->nMoneySupply > (uint64_t)(10000000000 + ((pindex->nHeight - 5000) * 4)) * COIN){
...
bool static ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace)
{
    ...
    LogPrint("bench", "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * 0.001, nTimeReadFromDisk * 0.000001);
    {
        ...
        dev::h256 oldHashStateRoot(globalState->rootHash()); // mmchain
        dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // mmchain
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot); // mmchain
            globalState->setRootUTXO(oldHashUTXORoot); // mmchain
...
//////////////////////////////////////////////////////////////////////////
    ...
    for (CBlockIndex* pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev)
    {
        ...
        ///////////////////////////////////////////////////////////////////// // mmchain
        uint32_t sizeBlockDGP = mmchainDGP.getBlockSize(pindex->nHeight);
        dgpMaxBlockSize = sizeBlockDGP ? sizeBlockDGP : dgpMaxBlockSize;
        updateBlockSizeParams(dgpMaxBlockSize);
        /////////////////////////////////////////////////////////////////////
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for MMChain PoS we need to sync this after every block to ensure txdb is populated for
