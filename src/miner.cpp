...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setMMChainSchedule(mmchainDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = mmchainDGP.getBlockSize(nHeight);
    minGasPrice = mmchainDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = mmchainDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    MMChainTxConverter convert(iter->GetTx(), NULL, &pblock->vtx);

    ExtractMMChainTX resultConverter;
    if(!convert.extractionMMChainTransactions(resultConverter)){
    ...
    for(MMChainTransaction mmchainTransaction : mmchainTransactions){
        txGas += mmchainTransaction.gas();
        if(txGas > txGasLimit) {
            // Limit the tx gas limit by the soft limit if such a limit has been specified.
            return false;
        }

        if(bceResult.usedGas + mmchainTransaction.gas() > softBlockGasLimit){
            //if this transaction's gasLimit could cause block gas limit to be exceeded, then don't add it
            return false;
        }
        if(mmchainTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, mmchainTransactions, hardBlockGasLimit);
...
void ThreadStakeMiner(CWallet *pwallet)
{
    ...
    RenameThread("mmchaincoin-miner");
...
void StakeMMChains(bool fStake, CWallet *pwallet)
{
