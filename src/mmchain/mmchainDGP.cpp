#include "mmchainDGP.h"

void MMChainDGP::initDataEIP158(){
...
bool MMChainDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData){
...
dev::eth::EVMSchedule MMChainDGP::getGasSchedule(unsigned int blockHeight){
    clear();
...
uint64_t MMChainDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t MMChainDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t MMChainDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t MMChainDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool MMChainDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void MMChainDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void MMChainDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void MMChainDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void MMChainDGP::createParamsInstance(){
...
dev::Address MMChainDGP::getAddressForBlock(unsigned int blockHeight){
...
void MMChainDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void MMChainDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void MMChainDGP::parseStorageOneUint64(uint64_t& value){
...
void MMChainDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = uint64_t(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule MMChainDGP::createEVMSchedule(){
    dev::eth::EVMSchedule schedule = dev::eth::EIP158Schedule;
