...
#include "mmchainstate.h"

using namespace std;
using namespace dev;
using namespace dev::eth;

MMChainState::MMChainState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = MMChainState::openDB(_path + "/mmchainDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

MMChainState::MMChainState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute MMChainState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, MMChainTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createMMChainAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            mmchain::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
...
std::unordered_map<dev::Address, Vin> MMChainState::vins() const // temp
{
...
void MMChainState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* MMChainState::vin(dev::Address const& _a) const
{
    return const_cast<MMChainState*>(this)->vin(_a);
}

Vin* MMChainState::vin(dev::Address const& _addr)
{
...
// void MMChainState::commit(CommitBehaviour _commitBehaviour)
// {
...
void MMChainState::kill(dev::Address _addr)
{
...
void MMChainState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
dev::Address MMChainState::createMMChainAddress(dev::h256 hashTx, uint32_t voutNumber){
...
void MMChainState::deleteAccounts(std::set<dev::Address>& addrs){
...
void MMChainState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void MMChainState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
