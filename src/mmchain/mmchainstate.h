...
#include <mmchain/mmchaintransaction.h>
...
class MMChainState : public dev::eth::State {
    
public:

    MMChainState();

    MMChainState(dev::u256 const& _accountStartNonce, dev::OverlayDB const& _db, const std::string& _path, dev::eth::BaseState _bs = dev::eth::BaseState::PreExisting);

    ResultExecute execute(dev::eth::EnvInfo const& _envInfo, dev::eth::SealEngineFace const& _sealEngine, MMChainTransaction const& _t, dev::eth::Permanence _p = dev::eth::Permanence::Committed, dev::eth::OnOpFunc const& _onOp = OnOpFunc());
...
private:
    ...
    dev::Address createMMChainAddress(dev::h256 hashTx, uint32_t voutNumber);
...
struct TemporaryState{
    std::unique_ptr<MMChainState>& globalStateRef;
    dev::h256 oldHashStateRoot;
    dev::h256 oldHashUTXORoot;

    TemporaryState(std::unique_ptr<MMChainState>& _globalStateRef) : 
...
public:
    ...
    CondensingTX(MMChainState* _state, const std::vector<TransferInfo>& _transfers, const MMChainTransaction& _transaction, std::set<dev::Address> _deleteAddresses = std::set<dev::Address>()) : transfers(_transfers), deleteAddresses(_deleteAddresses), transaction(_transaction), state(_state){}
...
private:
    ...
    const MMChainTransaction& transaction;
